...
export async function GET(request) {
  ...
  const dateParam = searchParams.get('date') || ''; // 'today' | 'yesterday' | 'tomorrow' or dates
  if (!trainNo) {
  ...
  try {
    ...
    const path = `/api/getTrainInfo/${trainNo}`;
    ...
    console.error("Train schedule proxy error:", err);
